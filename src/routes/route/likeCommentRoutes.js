...
router.route("/addComment").put( async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({success: true, response: newComment, message: "Thanks for comment"})
    } catch (error) {
        console.log(error)
        res.status(500).json({success: false, msg: "The comment wasn't added, try again"})
    }
})
...
router.route("/modifyComment").put( async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({ success: true, response: modifiedComment, message: "Comment edited successfully"})
    } catch (error) {
        console.log(error)
        res.status(500).json({success: false, msg: "The comment wasn't edited, try again"})
    }
})
...
router.route("/deleteComment").delete( async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({ success: true, response: { deletedComment }, message: "Comment deleted successfully"})
    } catch (error) {
        res.status(500).json({success: false, msg: "The comment wasn't deleted, try again"})
    }
})
